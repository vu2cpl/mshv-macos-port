...
| *(unreleased, 2026-09-10)* | **`azel.dat` is written to Library, not into the bundle.** `HvAstroDataW` built its path from `applicationDirPath()`, which on a Mac is `MSHV.app/Contents/MacOS/` — a directory with no `settings/` — so the `QFile` open in `astro0()` failed silently and the port never wrote the Moon/Sun Az/El + Doppler file that rotator and tracking programs read (nor could it have from a read-only DMG, or without breaking the seal of a signed bundle in `/Applications`). Now `mshv_app_data_path() + "/settings/azel.dat"` under `#if defined _MACOS_`, upstream line kept in the `#else` — the same shape as the `cty.dat` fix in `HvCty::ReadCtyDat`. The seed already ships `settings/azel.dat`, so the file lives at `~/Library/Application Support/MSHV/settings/azel.dat`, rewritten every 2 s while the Astronomical Data window is open. Verified by launching the build with a scratch `HOME`: the file was rewritten with the live UTC time 11 s after launch, nothing appeared under `Contents/MacOS/`. It was the last bundle-relative write in the tree. |
