...
| *(2026-09-17, unreleased)* | **Starting MSHV no longer disturbs other apps' audio.** At startup MSHV opened the Mac's default input device at 44.1 kHz, then the saved device at 44.1 kHz and again at 48 kHz, all before its settings were in. Each open sets that CoreAudio device's sample rate, so another program playing through the default device (an SDR client on a USB audio interface, for example) crackled and dropped out whenever MSHV started. `macsound_in.cpp` now opens the capture device once, on the first capture tick, with the final device and rate; nothing is opened for Flex Native or TCI input, and nothing until Monitor is on. Verified with a sample-rate watcher: one open, and no other device changed rate. |
| *(2026-09-09, reworked 2026-09-17)* | **Native Flex backend: start on the last frequency.** MSHV follows the rig, and a Flex hands a fresh GUI client a slice at its own 14.100 USB default, so MSHV used to adopt 20m from the radio and never tune. Now, once the backend is up and the rig is active and read, MSHV pushes the remembered frequency — or 14.074 with nothing remembered — through the from-App band-button path, which also re-syncs the band and sets DIGU. Since LZ2HV's rc31 (merged 2026-09-17) the push lives in the Flex panel and `HvTxW` (`HvTxW::UpdateFlexMeter`, retried each 500 ms panel tick while pending), replacing the first version's `EmitFlexSliceReady` → `Main_Ms::FlexPushStartFreq` chain, which is commented out as in his tree. Remembered in `ms_mesages` (it was `ms_settings`, so an update starts once on 14.074) — as `flex_native_last_freq` then, `flex_native_last_all` since the rc034 merge below — tracked only while the backend is running. Native-backend only; plain Flex CAT still follows the radio. `slice create` also gains `freq=`. Verified on a FLEX-6600: 18.100 / 14.074 / 21.074 DIGU from the saved value. |
| *(2026-09-15)* | **Flex native: fix a start-up race that could leave MSHV never connecting.** For a `FlexRadio SmartSDR Slice` rig, `Network::initAll()` sends `info` and then shows "Connecting … Try". Sending can re-enter `readNet()`; if the reply is read there, the CAT init completes and arms the deferred Flex start, and the "Connecting" status that follows stops CAT polling and calls `VitaStop()`, cancelling that start. The TCP session stays up and the radio answers, but no `client gui` is ever sent: no slice, audio or waterfall. `initAll()` now returns right after the send when the init has already completed. Traced and tested on LZ2HV's rc029 with the same change (8 of 8 starts connected, 7 through the re-entrant case); sent to him. |
| *(2026-09-18)* | **Flex native: the RX and TX antenna come back with the last frequency.** On a transverter band the radio changes the TX antenna itself (XVTA) but leaves the RX antenna on ANT1, so a station that last worked 2m started with no receive until the antenna was set by hand in the Flex panel (reported by Peter MM0CEZ). The panel now remembers both antennas while the backend is running and puts them back about a second after the start-up frequency is sent — after the retune, because the radio's own antenna change arrives with it. Saved on the existing frequency line as `frequency#rxant#txant` (the key was `flex_native_last_freq` then; LZ2HV renamed it in rc034, see below); storage format and the FlexPanel-only placement are LZ2HV's design. Start-up only: a band change later in the session still needs a pair per band, which is with him as a question. Verified on a FLEX-6600 by seeding an antenna the radio was not using: the slice moved to it half a second after the frequency landed, and a saved pair that already matches sends nothing. The transverter case itself needs a transverter to test. |
| *(2026-09-18)* | **Flex native: the antennas now follow the BAND.** The line above remembered one pair for the session, which a station with transverters cannot use: change band and the antennas stay where the last band left them, and the radio only ever does half the job — it moves the TX antenna itself onto a transverter band and leaves the RX antenna on the HF one, and coming back from 432 it moves nothing at all, so TX is left on the transverter port (all three cases reported by Peter MM0CEZ). The panel now keeps one RX/TX pair per band and puts that band's own pair back a second after every band change, on the same settings line: `14074000#ANT2#ANT2#8:ANT2:ANT2#18:XVTA:XVTA#20:RX_B:XVTB`, the first three fields unchanged so an older line still reads. A band nobody has used yet is left exactly as the radio set it up — nothing is guessed — and setting an antenna by hand in the panel cancels a restore that has not fired. Verified on a FLEX-6600 with two HF bands carrying different antennas: the band change moved both antennas 0.9 s after the frequency, and a band with nothing remembered was left alone and learned instead. |
| *(2026-09-18)* | **LZ2HV's rc034 merged: the Flex settings key is `flex_native_last_all`.** His rc034 carries the antenna save/restore, the per-band pairs and the antenna tuner as sent to him, and renames the `ms_mesages` key from `flex_native_last_freq` to `flex_native_last_all`. This tree follows: a `flex_native_last_freq=` line from an earlier build is no longer read and is dropped at the next save, so **every user starts once more on 14.074 and the antenna pairs are relearned** — nothing else is lost. The Flex panel source is his file again with only the port's own additions on top. Verified on a FLEX-6600 in a scratch HOME: the new key is pushed, no line gives 14.074, and a line under the old key is ignored and replaced. |
| *(2026-09-19)* | **Flex native: the per-band antennas are saved under the band's NAME.** The entries on the `flex_native_last_all` line were keyed by MSHV's band index (`8:ANT2:ANT2`); a band added to the table later would move every index after it, and everybody's antennas with them (LZ2HV's point, and his two-line sketch). They are now `14 MHz:ANT2:ANT2`, the same name the band menu shows — a user-defined band included. A line in the old index form is still read, once, and saved back under the names; a name this build does not have is dropped. Verified on the FLEX-6600 on his rc034 tree with the diff: name form restores and round-trips unchanged, the index form restores the same and is re-saved as names, an unknown name is dropped and the band learned. |
